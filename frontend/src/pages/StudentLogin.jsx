...
/**
 * IIITDM Jabalpur Official Hostel List
 */
...
function StudentLogin({ onBack, onLoginSuccess }) {
  const [formData, setFormData] = useState({ email: '', password: '', hostel: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await authAPI.studentLogin(formData.email.trim(), formData.password);

    setIsSubmitting(false);

    if (!result.success) {
      if (result.networkError) {
        setLoginStatus({ type: 'error', message: 'Cannot connect to server. Please ensure the backend is running on port 5000.' });
      } else {
        setLoginStatus({ type: 'error', message: result.message || 'Login failed. Please check your credentials.' });
      }
      return;
    }
    ...
    // Pass full student profile from DB to parent
    if (onLoginSuccess) {
