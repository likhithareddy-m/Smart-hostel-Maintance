...
import { getComplaintsByStudent, syncStudentComplaintsFromDb, COMPLAINT_STAGES } from '../services/complaintStorage.js';
 ...
 * 
 * @param {Object} props
 * @param {Object} props.student - Current logged-in student details
 * @param {Function} props.onBackToDashboard - Callback to return to student dashboard
 * @param {Function} props.onNavigateToReport - Callback to navigate to report a problem form
 */
function MyComplaints({ student, onBackToDashboard, onNavigateToReport }) {
  ...
  useEffect(() => {
    // Load complaints strictly registered by the currently logged-in student
    const studentEmail = student?.email || '';
    const data = getComplaintsByStudent(studentEmail);
    setComplaints(data);
    if (data.length > 0) {
    ...
    // Sync live records from Supabase PostgreSQL database
    syncStudentComplaintsFromDb(student?.id, studentEmail).then(synced => {
      if (synced && synced.length > 0) {
        setComplaints(synced);
        setSelectedComplaint(prev => (prev ? synced.find(c => c.id === prev.id) || synced[0] : synced[0]));
      }
    });
