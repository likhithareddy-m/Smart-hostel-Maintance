 ...
 * @param {Function} props.onLoginSuccess - Callback when admin successfully logs in
 */
function AdminLogin({ onBack, onLoginSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await authAPI.adminLogin(formData.email.trim(), formData.password);
    ...
    if (!result.success) {
      if (result.networkError) {
        ...
        setLoginStatus({
          type: 'error',
          message: result.message || 'Invalid admin credentials. Please try again.'
        });
      }
      return;
    }
    ...
    // Notify parent to transition to Admin Dashboard
    if (onLoginSuccess) {
      setTimeout(() => {
